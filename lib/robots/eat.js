...
module.exports = function(user, content, echo) {
    ...
    var types = ['午', '晚'];
    var entypes = ['Lunch ', 'Dinner '];
    var lunchReg = /午|lunch/ig;
    var dinnerReg = /晚|dinner/ig;
    ...
    if(allFoodsReg.test(content)) {
        var result = [];
        coreAction(user, 'Lunch ', function(str) {
            result.push(str);
            coreAction(user, 'Dinner ', function(str) {
                result.push(str);
                echo(result.join('\n'));
            });
        });
        ...
        if(cancelReg.test(content)) {
            ...
            if(isOver) {
                ...
                order.addOne(user, eatType, function(info) {
                    if(info.addnew) {
                        echo(entypes[eatType] + 'successfully reserved! 🍚');
                    } else {
